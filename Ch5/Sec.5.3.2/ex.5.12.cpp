// Q: Modify our vowel-counting program so that is counts the number of occurrences of the following two-character sequences: `ff`, `fl`, and `fi`.
...
int main() {
    ...
    cout << "ff occurs " << ffCnt << ((ffCnt == 1) ? "time" : "times");
    cout << "fl occurs " << ffCnt << ((ffCnt == 1) ? "time" : "times");
    cout << "fi occurs " << ffCnt << ((ffCnt == 1) ? "time" : "times");
    return 0;
}